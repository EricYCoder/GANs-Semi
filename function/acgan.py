...
# for my dataset rice:0 cotton:1
folderPath = os.path.join(home_dir, "Documents/semi_gan/GANs-Semi/images")
img_list = os.listdir(folderPath)
img_num = len(img_list)
train_data = np.empty((0, 1, 32, 32), np.int16)
train_label = np.empty((0, 1), np.int16)
for img_file in img_list:
    if img_file.split("_")[0] == "corn":
        label = 0
    elif img_file.split("_")[0] == "cotton":
        label = 1
    elif img_file.split("_")[0] == "rice":
        label = 2
    elif img_file.split("_")[0] == "soybean":
        label = 3
    else:
...
def sample_image(n_row, batches_done):
    ...
    labels = np.array([num % 2 for _ in range(n_row) for num in range(n_row)])
    labels = Variable(LongTensor(labels))
    gen_imgs = generator(z, labels)
    save_image(
        gen_imgs.data, "crop_images/%d.png" % batches_done, nrow=n_row, normalize=True
    )
